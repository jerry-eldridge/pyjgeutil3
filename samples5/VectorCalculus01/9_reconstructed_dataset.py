...
I_max = np.max(vv)
I_min = np.min(vv)
volume2 = np.zeros((N,M,D),dtype=np.uint8)
for k in range(D):
    for j in range(M):
        for i in range(N):
            I = volume[i,j,k]
            ...
            volume2[i,j,k] = val
for k in range(D):
    fn = f"{DATASET_DIR}data-{k:03d}.jpg"
    print(f"writing results to fn = {fn}")
    im = volume2[:,:,k]
    sz = 300 # sz x sz image
    scale = sz/N
    ww = int(N*scale)
    hh = int(M*scale)
    im2 = cv2.resize(im,(ww,hh))

    ## uncomment to save image slices as
    ## jpg images.
