...
def Isomorphic(doc1,doc2):
    ...
    E2 = doc2["E"]
    if (V1 == V2) and (E1 == E2):
        return True, list(range(n1))
    
    deg1 = [Degree(doc1,v) for v in doc1["V"]]
    deg2 = [Degree(doc2,v) for v in doc2["V"]]
    deg1.sort()
    deg2.sort()
    print(deg1)
    print(deg2)
...
def DeleteVertex(doc,x):
    ...
    sub_V = list(map(geth,doc['V']))
    try:
        sub_V.remove(None)
    except:
        i = 0
    sub_E = list(map(getg, list(range(len(doc['E'])))))
    try:
        sub_E.remove(None)
    except:
        i = 0
...
def ContractEdge(doc,e):
    ...
    u = up
    v = vp
    E = [equate(e,u,v) for e in E]
    # make E a set but will lose the order of edges and weights
    doc2["E"] = list(map(eval,list(set([str(e) for e in E]))))
...
def CreateCircleGeometry(doc,cx,cy,cz,r):
    ...
    for i in range(n):
        ...
        z = 0
        pt = [x,y,z]
        pt = list(map(int,list(map(round,pt))))
        doc2["pts"].append(pt)
    return doc2
...
def IntersectionGraph(S,sort=True):
    ...
    if sort:
        for i in range(len(S2)):
            S2[i].sort()
    T = set(map(str,S2))
    doc["V"] = list(range(len(T)))
    doc["E"] = []
    #Obj = []
    V = doc["V"]
    TL = list(T)
    doc["object"] = list(map(eval,TL))
    for u in V:
        for v in V:
            if u == v:
                continue
...
def imm(A,chi,verbose=False):
    ...
    n = sh[0]
    print(n)
    L = list(itertools.permutations(list(range(n)),n))
    S = 0
    for tup in L:
        if verbose:
            print(tup)
        ...
        if e in doc1["E"]:
            ...
            if midpt:
                try:
                    pt1 = doc1["pts"][u]
                    pt2 = doc1["pts"][v]
                    pt = list(map(int,vectors.lerp(pt1,pt2,0.4)))
                    flag = True
                except:
...
def Domain(R):
    domain = list(set([tup[0] for tup in R]))
    return domain
def Range(R):
    range0 = list(set([tup[1] for tup in R]))
    return range0
def PreImage(R,y):
    Ry = [tup for tup in R if tup[1]==y]
    return Ry
def Image(R,x):
    Rx = [tup for tup in R if tup[0]==x]
    return Rx 
...
def RelationGraph(R):
    ...
    domain = Domain(R)
    range0 = Range(R)
    V = list(range(len(domain)+len(range0)))
    E = []
    n = len(domain)
...
def RandomFunction(ndomain=10,nrange=10):
    V1 = list(range(ndomain))
    V2 = list(range(nrange))
    R = []
    for u in V1:
        v = random.choice(V2)
        R.append([u,v])
...
def Snm(n,m):
    ...
    G2 = Gnm(n,m)
    # top side
    V1 = [v for v in G2['V'] if G2['object'][v][1]==0]
    # bottom side
    V2 = [v for v in G2['V'] if G2['object'][v][1]==m-1]
    G1 = MergeVerticesPseudo(G2,V2)
    G = MergeVerticesPseudo(G1,V1)
    # left side
    V3 = [v for v in G2['V'] if G2['object'][v][0]==0]
    # right side
    V4 = [v for v in G2['V'] if G2['object'][v][0]==n-1]
