...
def grad(F,nabla_t = .2):
    def f1(x):
        ...
        for i in range(n):
            ...
            vec.append(val)
        g = np.array(vec)
        print(f"i = {i_grad}, F(a) = {F(x)}, |da| = {np.linalg.norm(g)}")
        i_grad = i_grad + 1
        return g
    return f1
...
def myfmin0(F,x0,gamma=10,xtol=1,maxiter=50,
           nabla_t = .2, c_max = 2,
           ftol = 0.05):
     a = np.array(x0)
...
def myfmin(F,x0,gamma=10,xtol=1,maxiter=50,
     ...
     iters = res.nit
     a = res.x
     print(f"Finish: i = {iters}, F(a) = {F(a)}")
     X_opt = a
     X_opt = list(map(c(c_max),X_opt))
     return X_opt, iters
