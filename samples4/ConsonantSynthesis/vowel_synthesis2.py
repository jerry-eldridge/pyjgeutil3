...
def create_sound(dur_secs, poles,dB0,
                 fn_save,
                 freq = 124,
                 vocal_range = [0,4000],
                 fn_spec_save = None,
    ...
    f_clip = vocal_range[-1]
    idx_lo = Get_idx(vocal_range[0],epsilon=150)
    idx_hi = Get_idx(vocal_range[1],epsilon=150)
    ...
    if fn_spec_save is not None:
        root = r"./txt/"
        fn_U = root+fn_spec_save + "-U.txt"
        fn_T = root+fn_spec_save + "-T.txt"
        fn_R = root+fn_spec_save + "-R.txt"
        fn_V = root+fn_spec_save + "-V.txt"
        fU = open(fn_U,'w')
        fT = open(fn_T,'w')
        fR = open(fn_R,'w')
        fV = open(fn_V,'w')
    ...
    for k in range(nchunks):
        ...
        if fn_spec_save is not None:
            for i in range(len(F)):
                if k == 1:
                    x_U = f'{F[i]:.6f}\t{dB(abs(U[i])):.6f}\n'
                    #print(x_U,end = ' ')
                    fU.write(x_U)
                    x_T = f'{F[i]:.6f}\t{dB(abs(T(F[i]))):.6f}\n'
                    #print(x_T,end = ' ')
                    fT.write(x_T)
                    x_R = f'{F[i]:.6f}\t{dB(abs(R(F[i]))):.6f}\n'
                    #print(x_R,end = ' ')
                    fR.write(x_R)
                    x_V = f'{F[i]:.6f}\t{dB(abs(V[i])):.6f}\n'
                    #print(x_V,end = ' ')
                    fV.write(x_V)
        ...
        for i in range(len(V2)):
            if i < idx_lo and i > min(idx_hi,n):
                V2[i] = 0
        # Um = |U|
        Vm = list(map(abs, V2))
        #Ulm = log(|U|)
