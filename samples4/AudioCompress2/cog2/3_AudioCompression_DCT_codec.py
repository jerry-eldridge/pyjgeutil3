...
def encode_audio_compress_sample2(x,j,chunk=None,
    ...
    eps = 100
    N = chunk
    n0 = Get_idx(chunk,sampling_rate)(2*f_c,eps)
    Y = list(y)
    if len(Y) < chunk:
        Y2 = list(Y)
        Y3 = [0]*(chunk-n0)
...
def Cog2Bytes2Wav(y_l,y_r,fn_wav2,
...
create_cog = True
if create_cog:
    y_l,y_r,sampling_rate = Wav2Cog2Bytes(fn_wav,
        chunk,f_c,zlib_level)
    SaveCog2(fn_cog,y_l,y_r,sampling_rate,chunk)

y_l2,y_r2,sampling_rate2,chunk2 = ReadCog2(fn_cog)
Cog2Bytes2Wav(y_l2,y_r2,fn_wav2,
