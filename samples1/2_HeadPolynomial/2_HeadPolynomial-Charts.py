...
def pyBezier4(pts):
	...
	def f(t):
	    ...
	    MP1 = sympy.Matrix(np.array([pt[0] for pt in pts]))
	    MP2 = sympy.Matrix(np.array([pt[1] for pt in pts]))
	    Mb = M*MP1
	    x = Mt*Mb
	    Mb = M*MP2
	    y = Mt*Mb
        ...
        while s <= 1.0+ds:
                ...
                if flag:
                        try:
                                ...
                                T2 = list(filter(viz, T2))
                                T2 = [z.real for z in T2]
                                r = lambda u: q1.subs(t2,u).as_poly(t1).all_coeffs()
                                T1 = list(map(r, T2))
                                T1 = [list(np.roots(C))[0] for C in T1]
                                Z = list(zip(T1,T2))
                                pts_Z = []
                                for z in Z:
                                        u1,u2 = z
