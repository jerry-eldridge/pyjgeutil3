...
def Transform(shape, T):
    shape4 = [pt+[0,1] for pt in shape]
    SHAPE4 = [list(np.einsum('ij,j->i',T,pt)) for pt in shape4]
    SHAPE = [pt[:2] for pt in SHAPE4]
    return SHAPE
...
def Center(shape):
    ...
    CM = sum([np.array(pt)/len(shape) for pt in shape])
    return CM
def Scale(shape,s):
...
p1,p2,p3,p4,p5 = list(map(MapPoints,[p1,p2,p3,p4,p5]))
G1,G2,G3,G4,G5 = list(map(PathGraph,[p1,p2,p3,p4,p5]))
