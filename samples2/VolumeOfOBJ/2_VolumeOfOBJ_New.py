...
G = OBJ2Graph(OBJS)
C = Center(G['pts'])
G = Translate(G,-np.array(C))
s = 0.8
G['pts'] = Scale(G['pts'],s,s,s)
print("len(G['V'])=",len(G['V']))
print("len(G['E'])=",len(G['E']))
print("len(G['F'])=",len(G['F']))
# 3D printer materials price in dollars
price_multicolor = 109.75
price_alumide = 98.44
price_polyamide = 70.02
price_rubberlike = 520.70
price_steel = 744.29
price_standard_resin = 271.74
price_gray_resin = 357.42
price_transparent_resin = 305.55
price_aluminum = 1024.57
price_ABS = 145.31
vol_calib = 152946.815363
def price(V,price0):
    rate = price0/(1.0*vol_calib)
    return V*rate
V = volume(G)
print("volume(G)=",V)
print("The calculated price and actual price differ greatly")
print("Approx. 3D Printer: price(G,multicolor)=",price(V,price_multicolor))
