...
def CoffeeCup0(n=30,m=20,r0=20.):
    ...
    if flag_cup:
         ...
         x_min = min([pt[0]-x0 for pt in curve1])
         x_max = max([pt[0]-x0 for pt in curve1])
         y_min = min([pt[1] for pt in curve1])
         y_max = max([pt[1] for pt in curve1])
         degrees = 0
         axis = [0,1,0]
         q = aff.HH.rotation_quaternion(degrees,axis[0],axis[1],axis[2])
         scale = 1.
    ...
    if flag_handle:
         ...
         # Assume curve has symmetry y-axis
         x0 = curve2[0][0]
         x_min = min([pt[0]-x0 for pt in curve2])
         x_max = max([pt[0]-x0 for pt in curve2])
         y_min = min([pt[1] for pt in curve2])
         y_max = max([pt[1] for pt in curve2])
...
################ MERGE BEGIN SCENE 1 and SCENE 2
...
ext.Graph2OBJ(BIGDATA+"CoffeeCupTable.obj",G,"scene")

# Double-Click on OBJ file
import os
print("Open CoffeeCupTable with 3D Viewer by double-clicking on it")
os.system(BIGDATA+"CoffeeCupTable.obj")
