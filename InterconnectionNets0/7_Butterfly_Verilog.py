...
def BuildPacket(G,path,data,stages,radix):
    start = path[0]
    goal = path[-1]
...
data = [1,1,0,0]
while not done:
    print("Input 'proc' for start and goal")
    print("Enter 'q' to quit")
    print("proc range: ",(jmin,jmax))
    s = input("start: proc = ")
    if s == 'q':
        break
    toks = s.split(' ')
    ...
    path = A_star_digraph((G1['V'],G1['E']),
                start,goal, cost)
    DisplayPath(G1,path)
    BuildPacket(G1,path,data,stages0,radix0)
    print()
