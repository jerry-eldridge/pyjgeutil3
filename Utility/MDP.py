...
class MDP:
    def __init__(self,start,S,V0,X,gamma=.1):
        ...
        self.X = deepcopy(X)
        self.A = list(range(len(X))) # set of actions indices ia
        self.P0 = list(zip(self.A,[L[1] for L in X]))
        self.R0 = list(zip(self.A,[L[2] for L in X]))
    ...
    def pi(self,s1):
        ...
        """
        Q_s1 = [self.Q(s1,ia) for ia in self.A]
        ia = np.argmax(Q_s1)
        return ia
    def UpdateV(self,s):
        """
    ...
    def GetQ(self,s):
        ...
        """
        Q_s = [self.Q(s,ia) for ia in self.A]
        return Q_s
    def Decide(self,s):
        """
