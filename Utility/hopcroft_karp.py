...
class HopcroftKarp:
    ...
    def DefineGraph(self):
        U0 = list(zip(self.Dom,[0]*len(self.Dom)))
        V0 = list(zip(self.Ran,[1]*len(self.Ran)))
        n1 = len(U0)
        n2 = len(V0)
        NIL = n1+n2
        Vdesc = U0 + V0 + [(NIL,2)]
        V = list(range(len(Vdesc)))
        E = []
        for i in V:
            x1,y1 = Vdesc[i]
            if y1 != 0:
        ...
        G['Vdesc'] = Vdesc
        G['E'] = E
        self.U = list(range(n1))
        self.V = list(range(n1,n1+n2))
        self.NIL = NIL
        return G
