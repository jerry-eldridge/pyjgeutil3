...
class q:
    ...
    def __str__(self):
        ...
        for m in M:
            t = '%s,' % m
            s = s + t
        s = s[:-1] + ',pts=%s)' % (str(self.points))
        self.L = s
        return self.L
    def insert(self, pt):
        if not self.boundary.containsPoint(pt):
            return False
        if (len(self.points) < self.QT_max) and \
    ...
    def subdivide(self):
        ...
        pts.append([c2x,c2y])
        for i in range(4):
            b = AABB(pts[i],self.boundary.hd/2)
...
print("query results: ", c.query(roi))
