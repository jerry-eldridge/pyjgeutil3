...
c = q(ROI2([1,1],ww-2,hh-2),capacity=4)
print(len(pts))
for idx in range(len(pts)):
    pt = pts[idx]
    c.insert(pt)
    gr.Point(pt,color=[0,0,0])
#print("c = \n",c)
while True:
    gr.Clear()
    w0,h0 = [75,75] # mouse window
    for idx in range(len(pts)):
        pt = pts[idx]
        gr.Circle(pt,3,color=[0,0,0],thickness=2)
        
    roi_mouse = AABB(pt_mouse,w0,h0)
    #print("roi = ",roi.Interval())
    pts_q,bboxes_q = c.query(roi_mouse)
    roi_select = AABB(pt_select,w0,h0)
    pts_s,bboxes_s = c.query(roi_select)
    if selected:
        print("selected = ",pts_s)
        selected = False
