...
def IneqPoint(A,B,O):
    M = np.array([[B[0]-A[0],O[0]-A[0]],
                  [B[1]-A[1],O[1]-A[1]]])
    m = -np.linalg.det(M)
    return m

def InteriorPoint(O, E,P):
    flag = True
    for i in range(len(E)):
        e = E[i]
        u,v = e
        ...
        if m <= 0:
            ...
            b = -b
            c = -c
        #s = '%.3f*x + %.3f*y <= %.3f' % (a,b,c)
        flag_i = a*O[0] + b*O[1] < c
        flag = flag and flag_i
    return flag

def DisplayIneq(E,P):
    O,area = PolygonCentroid(P)
    flag = True
    for i in range(len(E)):
        e = E[i]
        u,v = e
