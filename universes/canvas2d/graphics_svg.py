...
Drawline = lambda A,B,color="black" : Polyline([list(map(Screen_x,A)),list(map(Screen_y,B))],color)
def EllipseFill(s,pt,ra,rb,color="black"):
    x,y = pt
    t = list(range(101))
    xx = [ra*cos(2*pi*a/100)+x for a in t]
    yy = [rb*sin(2*pi*a/100)+y for a in t]
    pts = list(map(list,list(zip(xx,yy))))
