...
from .common import mapto
from .common import affine as aff
from .common import polygon_area as pa
from .common import a_star_digraph as asd

#import sys
#sys.path.insert(0,r"C:\_PythonJGE\Utility")
from .common import QuaternionGroup as HH
...
def Cube(ww,hh,dd,mass=1e8):
     ...
     # points to G
     pts = [[0,hh,0],[0,hh,dd],[ww,hh,dd],[ww,hh,0],
            [0,0,0],[0,0,dd],[ww,0,dd],[ww,0,0]]
     G['pts'] = pts
     ...
     F2 = [list(reversed(fi)) for fi in F]     
     G['F'] = F2
     G['N'] = [FaceNormal(G,v) for v in range(len(F))]
     return G
...
def area3d(G,i):
     face = G['F'][i]
     polygon1 = [G['pts'][v] for v in face]
     normal1 = G['N'][i]
...
def volume(G):
     ...
     for i in range(len(G2['F'])):
         ...
         n = G2['N'][i]
         face = G2['F'][i]
         polygon1 = [G2['pts'][v] for v in face]
         F = np.array(C)/3.0
...
shift0 = lambda A,n: [v+n for v in A]
shift1 = lambda A,n: [[v+n for v in f] for f in A]
...
def FloorStickPolygon(Gs,G,poly,width=1.,height=1.,closed=True):
     ...
     for i in range(m):
          ...
          t = deepcopy(A)
          # X'-axis
          wk,hk,dk =  list(map(f, [distance(A,B),height*16., width*16.]))
...
def ExtrudeGraph(doc,k):
     ...
     """
     doc1 = {"V":list(range(k)),"E":g.PathEdges(list(range(k)))}
...
def Extrusion0(G,path, spath, bcap=True,ecap=True,closed=False):
    ...
    if flag_a and use_tripy:
         ...
         pts_k = aff.Translate(pts_k,C_k[0],C_k[1],C_k[2],align=False)
         z_k = pts_k[0][2]
         pts_k = [pt[:2] for pt in pts_k]
         tris = Triangulate(pts_k)
         for tri in tris:
              f = [u + 0*m for u in tri]
              F.append(f)
              A,B,C = [pts_k[u] for u in tri]
